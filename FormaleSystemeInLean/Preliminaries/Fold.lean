...
theorem List.mem_fold_union_iff [DecidableEq β] { l: List (Finset β) }:
...
/--Theorem: An element`e`is in the set `𝔽`,

  where `𝔽`is the union of the sets
  ...
  in`s`.-/
theorem Finset.mem_fold_union_iff [DecidableEq β] { f: α → Finset β }:
